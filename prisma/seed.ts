...
const PRODUCT_CATEGORY = ["sneaker", "accessories", "allclothing"];
...
async function main() {
  ...
  await prisma.productCategory.createMany({
    data: PRODUCT_CATEGORY.map((category) => ({ name: category })),
    skipDuplicates: true, // Optional: skips if a category with the same name already exists
  });
  ...
  for (const product of PRODUCTS) {
    ...
    if (!category) {
      console.error(`Product category not found ${product.category}`);
      continue;
    }
    if (!subCategory) {
      console.error(`Product subcategory not found ${product.subCategory}`);
      continue;
    }
    try {
      await prisma.product.create({
        data: {
          ...
          otherImg: product.otherImgs?.slice(0, 2) || [],
          style: product.style,
